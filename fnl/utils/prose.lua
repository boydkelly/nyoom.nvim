...
function M.set_quotes()
  -- 1. Clear any existing mapping (removes your custom Guillemets function)
  clear_quote_map()
  -- 2. Handle nvim-autopairs
  local has_autopairs, autopairs = pcall(require, "nvim-autopairs")
  if has_autopairs then
    local Rule = require "nvim-autopairs.rule"
    -- Re-add the standard pair rule
    autopairs.add_rule(Rule('"', '"'))
  end

  -- 3. Handle mini.pairs
  -- We check if mini.pairs is loaded by looking for its global table or
  -- assuming it's the fallback if autopairs isn't there.
  if package.loaded["mini.pairs"] or not has_autopairs then
    -- Call your specific mini_pairs setup helper for standard quotes
    require("setup").mini_pairs "quotes"
  end

  vim.notify "Standard quote input restored"
end
